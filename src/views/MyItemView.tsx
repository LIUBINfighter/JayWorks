...
const DocsApp: React.FC = () => {
    ...
    try {
      ...
      if (rec.compiled) {
        ...
        for (const [k, v] of Object.entries(fm)) {
          if (v instanceof Date) normalized[k] = (v as Date).toISOString().slice(0, 10);
          else if (typeof v === "object" && v !== null) normalized[k] = JSON.stringify(v);
          else normalized[k] = v;
        }
        ...
        const { frontmatter: fm, element } = renderMdxToReact(rec.raw, {
  ...
  const navigateWithoutSearch = (id: string) => {
    ...
    const rootEl = document.querySelector(".jw-docs-content");
    if (rootEl) {
      clearHighlights(rootEl as HTMLElement).catch(() => {});
    }
    selectDoc(id);
  };
  ...
  const footer = (
    <div className="jw-docs-footer-bar">
      <div className="jw-footer-left">
        {leftWidgets.map((w) => (
          <React.Fragment key={w.id}>{w.render(ctxFactory())}</React.Fragment>
        ))}
      </div>
    ...
    <ShellLayout
      sidebar={
        <Sidebar
          ...
          docs={groupDocs}
          groupId={currentGroup}
          onSelect={navigateWithoutSearch}
          locale={activeLocale}
        />
      }
      header={
            ...
            <LocaleSwitcher
              onChange={(loc) => {
                ...
                if (currentId) {
                  const rec = docsRegistry.getDoc(currentId);
                  if (!rec) {
                    const firstInGroup = docsRegistry
                      .list()
                      .find((r) => r.meta.groupId === currentGroup);
        ...
        {!loading && !error && (
          <div>
            ...
            {useMemo(() => {
              ...
              if (!path) return null;
              return (
                <div className="jw-doc-path" style={{ marginBottom: 8 }} title={path}>
                  <code>{path}</code>
                </div>
              );
            ...
            {frontmatter.description && (
              <p style={{ marginTop: -8, opacity: 0.8 }}>{frontmatter.description}</p>
            )}
            <div
              style={{
    ...
    if (this.state.hasError)
      return (
        <div style={{ color: "var(--text-error)" }}>
          渲染出现错误: {String(this.state.error?.message || this.state.error)}
        </div>
      );
