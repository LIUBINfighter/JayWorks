...
export const CodeBlock: React.FC<CodeBlockProps> = ({ lang, code, html }) => {
    ...
    (async () => {
      try {
        ...
        const { svg } = await mermaid.render(
          "jw_mermaid_" + Math.random().toString(36).slice(2),
          code,
        );
        if (!cancelled) {
          setSvgHtml(svg);
  ...
  const handleCopyImage = async () => {
    ...
    try {
      ...
      const svgEl =
        rootEl && rootEl.tagName.toLowerCase() === "svg"
          ? (rootEl as unknown as SVGSVGElement)
          : null;
  ...
  return (
    <div
      className={"jw-codeblock" + (isMermaid ? " jw-codeblock-mermaid" : "")}
      data-lang={lang || ""}
    >
            ...
            <>
              ...
              >
                {imgCopied ? (
                  ...
                  <ImageIcon size={14} className={imgCopying ? "jw-anim-pulse" : ""} />
                )}
              </button>
            </>
          )}
      ...
      <div className="jw-codeblock-panels">
        ...
        {isMermaid && (
          <div className="jw-codeblock-diagram" data-mode="diagram" data-active={tab === "diagram"}>
            ...
            {!diagramError && !svgHtml && (
              <div className="jw-codeblock-diagram-loading">渲染中...</div>
            )}
            {!diagramError && svgHtml && (
              <div
                ref={svgContainerRef}
                className="jw-codeblock-diagram-svg"
                dangerouslySetInnerHTML={{ __html: svgHtml }}
              />
            )}
            {imgCopyError && (
              <div className="jw-codeblock-diagram-error" style={{ marginTop: 8 }}>
                {imgCopyError}
              </div>
            )}
          </div>
