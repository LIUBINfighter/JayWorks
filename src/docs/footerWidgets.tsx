...
export function getFooterWidgets(): FooterWidget[] {
...
registerFooterWidget({
  ...
  render: ({ doc, select }: FooterWidgetContext) => {
    ...
    const idx = canonicalList.findIndex(
      (r) => (r.meta.canonicalId || r.meta.id) === currentCanonical,
    );
    ...
    const groupLabel = (gid?: string) =>
      gid ? NAV_GROUPS.find((g) => g.id === gid)?.label || gid : "";
...
registerFooterWidget({
  ...
  order: 9,
  align: "right",
  render: ({ doc }: FooterWidgetContext) => <FooterMeta doc={doc} />,
});
